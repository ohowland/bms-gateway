...
alarm_re = re.compile(r"^\w+_(ARRIVE|LEAVE)$")
        ...
        for msg_name, sigs in from_msg.items():
            for sig_name, sig_val in sigs.items():
                ...
                if to_msg:
                    ...
                    LOGGER.debug(
                        "CAN message %s::%s is not mapped, consider removing from production dbc",
                        msg_name,
                        sig_name,
                    )
...
class Map:
                ...
                if match:
                    if match.group(1) == "ARRIVE":
                        ...
                        to_msgs = Map._write_sig_val(
                            msg_name,
                            sig_name,
                            int(not sig_val), # flip the bit, leave == !arrive
                            to_msgs,
                        )
                    else:
    ...
    def _write_sig_val(msg_name, sig_name, sig_value, to_msgs):
        ''' returns a mutated to_msgs which contains an updated msg
            with the signal value
        '''

        existing_msg = to_msgs.get(msg_name, None)
        if existing_msg:
            #print("EXISTS: msg_name {}, sig_name {}\n".format(msg_name, sig_name))
            to_msgs[msg_name][sig_name] = sig_value
        else:
            #print("DOES NOT EXIST msg_name {}, sig_name {}\n".format(msg_name, sig_name))
            to_msgs[msg_name] = {sig_name: sig_value}

        #print("TO_MSGS {}".format(to_msgs))
        return to_msgs
